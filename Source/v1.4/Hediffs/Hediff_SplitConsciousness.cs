...
namespace SkyMind
{
    ...
    public class Hediff_SplitConsciousness : HediffWithComps
    {
        ...
        private void SetSeverity(int pawnCount)
        {
            ...
            {
                cachedSurrogateSoftCap = SkyMindNetwork_Settings.safeSurrogateConnectivityCountBeforePenalty;
                // Surrogates check their controller for a surrogate limit bonus to add to the soft cap.
                if (SMN_Utils.IsSurrogate(pawn))
                {
                    cachedSurrogateSoftCap += (int)pawn.GetComp<CompSkyMindLink>().GetSurrogates().First().GetStatValue(SMN_StatDefOf.SMN_SurrogateLimitBonus);
                }
                // Controllers check their own surrogate limit bonus to add to the soft cap.
                else
                {
                    cachedSurrogateSoftCap += (int)pawn.GetStatValue(SMN_StatDefOf.SMN_SurrogateLimitBonus);
                }
            }

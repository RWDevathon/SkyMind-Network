...
namespace SkyMind
{
    public class CompHeatSensitive : ThingComp
    {
        ...
        {
            base.PostExposeData();
            Scribe_Values.Look(ref heatLevel, "SMN_heatLevel", 0, false);
            Scribe_Values.Look(ref checksSinceCritical, "SMN_checksSinceCritical", 0, false);
        }
        ...
        private void CheckTemperature(int tickerType)
        {
            ...
            if (ambientTemperature >= Props.dangerHeat)
            {
                ...
                if (lastTickSentCriticalHeat + 8000 < Find.TickManager.TicksGame)
                {
                    ...
                    Messages.Message("SMN_AlertServerHeatCriticalDesc".Translate(), new LookTargets(parent), MessageTypeDefOf.NegativeEvent, false);
                }
            ...
            if (checksSinceCritical >= 3750 + Rand.RangeInclusive(-1000, 1250))
            {
                ...
                MakeExplosion();
                Find.LetterStack.ReceiveLetter("SMN_ServerMeltdown".Translate(), "SMN_ServerMeltdownDesc".Translate(), LetterDefOf.NegativeEvent, new TargetInfo(parent.Position, parent.Map, false), null, null);
            }
        }
        ...
        public override string CompInspectStringExtra()
        {
            ...
            if (heatLevel == 3)
                return "SMN_CompHotSensitiveCriticalText".Translate();
            else if (heatLevel == 2)
                return "SMN_CompHotSensitiveDangerText".Translate();
            else if (heatLevel == 1)
                return "SMN_CompHotSensitiveWarningText".Translate();
            else
                return "SMN_CompHotSensitiveSafeText".Translate();
        }

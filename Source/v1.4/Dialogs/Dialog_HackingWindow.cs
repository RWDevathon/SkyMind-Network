...
namespace SkyMind
{
    public class Dialog_HackingWindow : Window
    {
        ...
        public Dialog_HackingWindow()
        {
            ...
            cachedHackPenalty = SMN_Utils.gameComp.hackCostTimePenalty;
            ResetCostModifiers();
        }
        ...
        public override void DoWindowContents(Rect inRect)
        {
            ...
            prelist.Label("SMN_HackInterfaceTitle".Translate());
            prelist.GapLine();
            ...
            availableHackingPoints = SMN_Utils.gameComp.GetPoints(SMN_ServerType.HackingServer);

            // Guidance Hack operation
            HackSelector(listingStandard, maxWidth, "SMN_GuidanceHack".Translate(), ref guidanceCostModifier, guidanceCostBase, OperationSuccessChance(guidanceSuccessChanceBase, guidanceCostBase, guidanceCostModifier), "SMN_GuidanceHackDesc".Translate(), delegate
            {
                ...
                foreach (Faction targetFaction in Find.FactionManager.GetFactions(minTechLevel: TechLevel.Industrial))
                {
                    ...
                    {
                        Find.WindowStack.Add(new Dialog_MessageBox("SMN_GuidanceHackDesc".Translate(), "Confirm".Translate(), buttonBText: "Cancel".Translate(), title: "SMN_GuidanceHack".Translate(), buttonAAction: delegate
                        {
                            ...
                            else // Succeeded
                            {
                                ...
                                if (randPick < 0.4f)
                                {
                                    ...
                                    {
                                        def = SMN_IncidentDefOf.ResourcePodCrash,
                                ...
                                else if (randPick < 0.75f)
                                {
                                    incident = new FiringIncident
                                    {
                                        def = SMN_IncidentDefOf.RefugeePodCrash,
                                        parms = StorytellerUtility.DefaultParmsNow(IncidentCategoryDefOf.Misc, Find.CurrentMap)
                                    };
                                }
                                // Ship chunk drop
                                ...
                                else if (randPick < 0.97f)
                                {
                                    incident = new FiringIncident
                                    {
                                        def = SMN_IncidentDefOf.PsychicEmanatorShipPartCrash,
                                        parms = StorytellerUtility.DefaultParmsNow(IncidentCategoryDefOf.ThreatBig, Find.CurrentMap)
                                    };
                                    incident.parms.points /= 2;
                                }
                                ...
                                else
                                {
                                    incident = new FiringIncident
                                    {
                                        def = SMN_IncidentDefOf.DefoliatorShipPartCrash,
                                        parms = StorytellerUtility.DefaultParmsNow(IncidentCategoryDefOf.ThreatBig, Find.CurrentMap)
                                    };
                                    incident.parms.points /= 2;
                                }
                                ...
                                {
                                    Messages.Message("SMN_NoArrival".Translate(), MessageTypeDefOf.NeutralEvent, false);
                                }
                            }
                            cachedHackPenalty += (int)guidanceCostBase;
                            SMN_Utils.gameComp.hackCostTimePenalty += (int)guidanceCostBase;
                            SMN_Utils.gameComp.ChangeServerPoints(-guidanceCostModifier, SMN_ServerType.HackingServer);
                            ResetCostModifiers();
                        }));
                    }));
            ...
            HackSelector(listingStandard, maxWidth, "SMN_PropagandaHack".Translate(), ref propagandaCostModifier, propagandaCostBase, OperationSuccessChance(propagandaSuccessChanceBase, propagandaCostBase, propagandaCostModifier), "SMN_PropagandaHackDesc".Translate(), delegate
            {
                List<FloatMenuOption> opts = new List<FloatMenuOption>();
                ...
                foreach (Faction targetFaction in Find.FactionManager.GetFactions(minTechLevel: TechLevel.Industrial).Where(faction => faction.HasGoodwill && faction.GoodwillWith(Faction.OfPlayer) > -100))
                {
                    opts.Add(new FloatMenuOption(targetFaction.Name, delegate
                    {
                        Find.WindowStack.Add(new Dialog_MessageBox("SMN_PropagandaHackDesc".Translate(), "Confirm".Translate(), buttonBText: "Cancel".Translate(), title: "SMN_PropagandaHack".Translate(), buttonAAction: delegate
                        {
                            // A faction has been selected for targetting. Check if the hack was successful. Handle failure and success separately.
                            ...
                            SMN_Utils.gameComp.hackCostTimePenalty += (int)propagandaCostBase;
                            SMN_Utils.gameComp.ChangeServerPoints(-propagandaCostModifier, SMN_ServerType.HackingServer);
                            ResetCostModifiers();
                        }));
                    }));
                    opts.SortBy((x) => x.Label);
            ...
            HackSelector(listingStandard, maxWidth, "SMN_EspionageHack".Translate(), ref espionageCostModifier, espionageCostBase, OperationSuccessChance(espionageSuccessChanceBase, espionageCostBase, espionageCostModifier), "SMN_EspionageHackDesc".Translate(), delegate
            {
                List<FloatMenuOption> opts = new List<FloatMenuOption>();
                ...
                foreach (Faction targetFaction in Find.FactionManager.GetFactions(minTechLevel: TechLevel.Industrial))
                {
                    opts.Add(new FloatMenuOption(targetFaction.Name, delegate
                    {
                        Find.WindowStack.Add(new Dialog_MessageBox("SMN_EspionageHackDesc".Translate(), "Confirm".Translate(), buttonBText: "Cancel".Translate(), title: "SMN_EspionageHack".Translate(), buttonAAction: delegate
                        {
                            // A faction has been selected for targetting. Check if the hack was successful. Handle failure and success separately.
                            ...
                            SMN_Utils.gameComp.hackCostTimePenalty += (int)espionageCostBase;
                            SMN_Utils.gameComp.ChangeServerPoints(-espionageCostModifier, SMN_ServerType.HackingServer);
                            ResetCostModifiers();
                        }));
                    }));
                    opts.SortBy((x) => x.Label);
            ...
            HackSelector(listingStandard, maxWidth, "SMN_DisruptorHack".Translate(), ref disruptorCostModifier, disruptorCostBase, OperationSuccessChance(disruptorSuccessChanceBase, disruptorCostBase, disruptorCostModifier), "SMN_DisruptorHackDesc".Translate(), delegate
            {
                List<FloatMenuOption> opts = new List<FloatMenuOption>();
                ...
                foreach (Faction targetFaction in Find.FactionManager.GetFactions(minTechLevel: TechLevel.Industrial).Where(faction => faction.HostileTo(Faction.OfPlayer) && faction.def.GetModExtension<SMN_FactionSkyMindExtension>()?.canUseSurrogates == true))
                {
                    ...
                    if (targetPawns.Any())
                    {
                        opts.Add(new FloatMenuOption(targetFaction.Name, delegate
                        {
                            Find.WindowStack.Add(new Dialog_MessageBox("SMN_DisruptorHackDesc".Translate(), "Confirm".Translate(), buttonBText: "Cancel".Translate(), title: "SMN_DisruptorHack".Translate(), buttonAAction: delegate
                            {
                                // A faction has been selected for targetting. Check if the hack was successful. Handle failure and success separately.
                                ...
                                else // Succeeded
                                {
                                    ...
                                    {
                                        Hediff ddosHediff = HediffMaker.MakeHediff(SMN_HediffDefOf.SMN_DDOSRecovery, pawn, pawn.health.hediffSet.GetBrain());
                                        ddosHediff.Severity = 1;
                                        pawn.health.AddHediff(ddosHediff, pawn.health.hediffSet.GetBrain());
                                    }

                                    Messages.Message("SMN_DisruptorHackSuccess".Translate(), MessageTypeDefOf.PositiveEvent, false);
                                }
                                cachedHackPenalty += (int)disruptorCostBase;
                                SMN_Utils.gameComp.hackCostTimePenalty += (int)disruptorCostBase;
                                SMN_Utils.gameComp.ChangeServerPoints(-disruptorCostModifier, SMN_ServerType.HackingServer);
                                ResetCostModifiers();
                            }));
                        }));
                        opts.SortBy((x) => x.Label);
            ...
            postlist.Label("SMN_AvailableHackingPoints".Translate(availableHackingPoints, cachedHackPenalty));
            postlist.Label("SMN_AvailableHackingPointsDesc".Translate());
        ...
        protected void HackSelector(Listing_Standard listingStandard, float sectionWidth, string title, ref float costModifier, float baseCost, float successChance, string operationDescription, Action operation)
        {
            ...
            if (baseCost + cachedHackPenalty <= availableHackingPoints)
            {
                ...
                if (listingStandard.ButtonText("SMN_SuccessChance".Translate(successChance * 100), operationDescription))
                {
                    operation.Invoke();
                }
            }
            else
                listingStandard.Label("SMN_InsufficientPoints".Translate(baseCost + cachedHackPenalty));
        ...
        protected void HandleOperationFailure(Faction faction)
        {
            ...
            if (Rand.Chance(SkyMindNetwork_Settings.retaliationChanceOnFailure))
            {
                ...
                if (retaliationPick < 0.5f && faction.HostileTo(Faction.OfPlayer))
                {
                    ...
                    if (!Find.Storyteller.TryFire(incident))
                    {
                        ...
                        incident = new FiringIncident
                        {
                            def = SMN_IncidentDefOf.SMN_HackingIncident,
                ...
                {
                    Quest quest = QuestUtility.GenerateQuestAndMakeAvailable(SMN_QuestScriptDefOf.ProblemCauser, StorytellerUtility.DefaultThreatPointsNow(Find.CurrentMap));
                    if (SMN_QuestScriptDefOf.ProblemCauser.sendAvailableLetter)
                    {
                        QuestUtility.SendLetterQuestAvailable(quest);
                    }
                    else
            ...
            {
                Messages.Message("SMN_HackOperationFailed".Translate(), MessageTypeDefOf.NegativeEvent, false);
            }
        }
        ...
        private void RelationshipRetaliation(Faction faction)
        {
            ...
            {
                Find.LetterStack.ReceiveLetter("SMN_RelationshipRetaliation".Translate(), "SMN_RelationshipRetaliationDesc".Translate(goodwillImpact), LetterDefOf.NegativeEvent);
            }
        }
        ...
        {
            return Mathf.Clamp(baseSuccessChance * (costModifier / (baseCost + cachedHackPenalty)) * RecentHackSuccessPenalty.Evaluate(cachedHackPenalty), SkyMindNetwork_Settings.minHackSuccessChance, SkyMindNetwork_Settings.maxHackSuccessChance);
        }

...
namespace SkyMind
{
    ...
    public class WorkGiver_GenerateInsight : WorkGiver_Scanner
    {
        ...
        {
            if (SMN_Utils.gameComp.GetPointCapacity(SMN_ServerType.SkillServer) <= 0 && SMN_Utils.gameComp.GetPointCapacity(SMN_ServerType.SecurityServer) <= 0 && SMN_Utils.gameComp.GetPointCapacity(SMN_ServerType.HackingServer) <= 0)
            {
                return true;
            }
            return false;
        ...
        public override bool HasJobOnThing(Pawn pawn, Thing t, bool forced = false)
        {
            ...
            if (compInsightBench == null || SMN_Utils.gameComp.GetPointCapacity(compInsightBench.SMN_ServerType) <= SMN_Utils.gameComp.GetPoints(compInsightBench.SMN_ServerType))
            {
                return false;
            }
        ...
        {
            return JobMaker.MakeJob(SMN_JobDefOf.SMN_GenerateInsight, t);
        }
    }
}

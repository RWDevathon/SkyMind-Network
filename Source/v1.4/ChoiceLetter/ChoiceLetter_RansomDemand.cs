...
namespace SkyMind
{
    public class ChoiceLetter_RansomDemand : ChoiceLetter
    {
                ...
                else
                {
                    ...
                    DiaOption accept = new DiaOption("RansomDemand_Accept".Translate())
                    {
                        action = delegate
                        {
                            ...
                            SMN_Utils.RemoveViruses(cryptolockedThings);
                            Messages.Message("SMN_GridlockerCleared".Translate(), MessageTypeDefOf.PositiveEvent);
        ...
        {
            base.ExposeData();
            Scribe_Collections.Look(ref cryptolockedThings, "SMN_cryptolockedThings", LookMode.Value);
            Scribe_Values.Look(ref fee, "SMN_cryptoFee", 0, false);
            Scribe_Values.Look(ref deviceType, "SMN_deviceType", false, false);
        }
    }
}